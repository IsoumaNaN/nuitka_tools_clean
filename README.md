...
Repo kèm sẵn skill `nuitka-dump/` và `nuitka-reverse/ (chứa `SKILL.md`) — quy trình dump + static reversing Nuitka để Claude Code tự làm theo (one-click dump, đọc output, và kỹ thuật `mod_consts[]` để map constant string về hàm C trong IDA).
...
cp -r nuitka-reverse ~/.claude/skills/
```
...
Copy-Item -Recurse -Force nuitka-reverse "$env:USERPROFILE\.claude\skills\"
```

Mở session Claude Code mới rồi gõ `/nuitka-dump`,`/nuikta-reverse`, hoặc cứ nhắc tới việc dump/reverse Nuitka là skill tự kích hoạt. Skill chỉ là text hướng dẫn — để bước dump chạy thật vẫn cần clone repo này + `pip install -r requirements.txt` + Python ≥ 3.10.
